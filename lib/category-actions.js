...
export async function createCategory(rawName) {
  ...
  // owner_id 는 NOT NULL 이고 categories_insert 정책도 owner_id = auth.uid() 를
  // 요구합니다. createSet 과 동일하게 여기서 명시적으로 넣습니다.
  const { data, error } = await supabase
    .from('categories')
  ...
  if (error) {
    ...
    // (다른 사람이 만든 카테고리도 공용이라 그대로 쓸 수 있습니다)
    if (error.code === '23505') {
    ...
    console.error('카테고리 생성 실패:', { error, ownerId: user.id, name });
