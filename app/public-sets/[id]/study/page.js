...
export default async function StudyPublicSetPage({ params, searchParams }) {
  const { id } = params;
  ...
  if (mode !== 'study' && mode !== 'memorize') {
    redirect(`/public-sets/${id}`);
  }

  const supabase = createServerSupabase();
  const bundle = await getQuestionSetForStudy(supabase, id);
  if (!bundle || !bundle.set.is_public) notFound();
    ...
    return (
      <>
        ...
        <Card padding="var(--space-5)" style={{ textAlign: 'center' }}>
          ...
          <Link href={`/public-sets/${id}`} style={{ textDecoration: 'none' }}>
  ...
  return (
    <>
      <h1 style={{ marginBottom: 4, fontSize: 20 }}>{bundle.set.title}</h1>
      <p className="muted" style={{ marginBottom: 'var(--space-5)' }}>
        by {bundle.set.owner?.username ?? '알 수 없음'}
      </p>
      <StudyView
        questions={bundle.questions}
        mode={mode}
        initialIndex={initialIndex}
        backHref={`/public-sets/${id}`}
      />
    </>
  );
}
